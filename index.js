...
function addLift(numberOfLifts) {
  ...
  for (let i = 0; i < numberOfLifts; i++) {
    addedLifts += `<div class="lift" id="lift" data-liftposition="0" >
...
function startLift(targetFloor, freeLift) {
  const currentPosition = parseInt(freeLift.dataset.liftposition);
  const time = Math.abs(targetFloor - currentPosition);
  const direction = targetFloor > currentPosition ? 'up' : 'down';
